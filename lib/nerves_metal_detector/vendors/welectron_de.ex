defmodule NervesMetalDetector.Vendors.WelectronDe do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.WelectronDe.ProductUpdate do
  alias NervesMetalDetector.Vendors.WelectronDe

  def fetch_availability(%WelectronDe.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: WelectronDe.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
