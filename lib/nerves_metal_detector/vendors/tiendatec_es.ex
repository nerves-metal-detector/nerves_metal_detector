defmodule NervesMetalDetector.Vendors.TiendatecEs do
  ...
  def vendor_info() do
    %Vendor{
      ...
      name: "Tiendatec",
      country: :es,
      homepage: "https://www.tiendatec.es"
    }
  end
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.TiendatecEs.ProductUpdate do
  alias NervesMetalDetector.Vendors.TiendatecEs
  ...
  def fetch_availability(%TiendatecEs.ProductUpdate{url: url, sku: sku}) do
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: TiendatecEs.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
