defmodule NervesMetalDetector.Vendors.SeeedStudioCn do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.SeeedStudioCn.ProductUpdate do
  alias NervesMetalDetector.Vendors.SeeedStudioCn

  def fetch_availability(%SeeedStudioCn.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: SeeedStudioCn.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
