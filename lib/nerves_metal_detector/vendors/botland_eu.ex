defmodule NervesMetalDetector.Vendors.BotlandEu do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.BotlandEu.ProductUpdate do
  alias NervesMetalDetector.Vendors.BotlandEu

  def fetch_availability(%BotlandEu.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: BotlandEu.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
