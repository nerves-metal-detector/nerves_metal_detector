defmodule NervesMetalDetector.Vendors.PiShopUs do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.PiShopUs.ProductUpdate do
  alias NervesMetalDetector.Vendors.PiShopUs

  def fetch_availability(%PiShopUs.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: PiShopUs.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
