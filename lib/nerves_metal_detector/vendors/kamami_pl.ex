defmodule NervesMetalDetector.Vendors.KamamiPl do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.KamamiPl.ProductUpdate do
  alias NervesMetalDetector.Vendors.KamamiPl

  def fetch_availability(%KamamiPl.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: KamamiPl.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
  ...
  defp parse_items_in_stock(html_tree) do
    with available when available not in [nil, []] <-
           Floki.find(html_tree, ".product-stock-k .product-stock-k-dlg"),
