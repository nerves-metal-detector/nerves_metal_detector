defmodule NervesMetalDetector.Vendors.ReicheltDe do
  ...
  def vendor_info() do
    %Vendor{
      ...
      name: "Reichelt",
      country: :de,
      homepage: "https://www.reichelt.de"
    }
  end
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.ReicheltDe.ProductUpdate do
  alias NervesMetalDetector.Vendors.ReicheltDe

  def fetch_availability(%ReicheltDe.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: ReicheltDe.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
