defmodule NervesMetalDetector.Vendors.PiShopCa do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.PiShopCa.ProductUpdate do
  alias NervesMetalDetector.Vendors.PiShopCa

  def fetch_availability(%PiShopCa.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: PiShopCa.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
