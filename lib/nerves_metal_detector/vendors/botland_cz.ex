defmodule NervesMetalDetector.Vendors.BotlandCz do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.BotlandCz.ProductUpdate do
  alias NervesMetalDetector.Vendors.BotlandCz

  def fetch_availability(%BotlandCz.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: BotlandCz.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
