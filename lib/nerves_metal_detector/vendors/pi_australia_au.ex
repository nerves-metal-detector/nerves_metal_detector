defmodule NervesMetalDetector.Vendors.PiAustraliaAu do
  ...
  def vendor_info() do
    %Vendor{
      ...
      name: "Pi Australia",
      country: :au,
      homepage: "https://raspberry.piaustralia.com.au"
    }
  end
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.PiAustraliaAu.ProductUpdate do
  alias NervesMetalDetector.Vendors.PiAustraliaAu

  def fetch_availability(%PiAustraliaAu.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: PiAustraliaAu.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
