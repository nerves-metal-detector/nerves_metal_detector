defmodule NervesMetalDetector.Vendors.BotlandDe do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.BotlandDe.ProductUpdate do
  alias NervesMetalDetector.Vendors.BotlandDe

  def fetch_availability(%BotlandDe.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: BotlandDe.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
