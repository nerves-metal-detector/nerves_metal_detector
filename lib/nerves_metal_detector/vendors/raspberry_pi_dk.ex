defmodule NervesMetalDetector.Vendors.RaspberryPiDk do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.RaspberryPiDk.ProductUpdate do
  alias NervesMetalDetector.Vendors.RaspberryPiDk

  def fetch_availability(%RaspberryPiDk.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: RaspberryPiDk.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
  ...
  defp find_offer(json_info, url) do
    with product when not is_nil(product) <-
           Enum.find(json_info["@graph"], &(&1["@type"] === "Product")),
         offers when is_list(offers) <- product["offers"] do
